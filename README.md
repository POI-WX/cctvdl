...
- **栏目链接** — 按月份浏览和搜索节目；可一键定位最早或最新节目月份、下载本月，也可跨月份和栏目批量下载
- **历史栏目归档** — 支持央视中途更换栏目 ID 的长期节目；粘贴节目概览或新旧任意一期，都会合并为同一个按月栏目
- **节目集链接** — 电视剧、纪录片和 4K 节目可从节目首页或任意一集导入；直接查看全部选集，并可切换从早到晚或从晚到早排列
- **单视频链接** — 电影、新闻文章中的视频、独立导视片段和央视新闻视频可直接加入下载列表；能识别所属节目集的片段会打开对应节目
...
- **可选扩展内容** — 可在设置中开启「加载节目看点和片段」，让节目列表同时显示正片、看点和片段
- **收藏与备份** — 常用内容可收藏置顶；可导出备份文件，也可随时导入恢复

**⬇️ 下载与队列**

- **画质选择** — 流畅、标清、高清、超清和蓝光对应固定带宽上限；在上限内选择最佳可用流，无符合项时使用最低带宽流，最高支持 1080p；CCTV-16 优先使用可用的明文 HLS
