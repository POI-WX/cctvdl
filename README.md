...
- **节目浏览** — 导入栏目、按月浏览、关键词搜索、封面预览、节目简介；月份选择器标注有内容月份
- **单视频直链** — 粘贴不属于任何栏目的独立视频（如电影）链接，直接下载并持久收藏
- **栏目管理** — 收藏置顶常用栏目、悬停快捷删除、批量清空、JSON 导入 / 导出备份
- **批量下载** — 一键下载栏目本月全部、多任务队列，实时显示速度 / 剩余时间 / 进度
- **并行下载** — 最多同时下载 3 个视频，线程数自动均分，总并发不变
- **清晰度选择** — 流畅 / 标清 / 高清 / 超清 / 蓝光（最高 1080p），或「自动」取最高；缺档自动降级
- **快速合并** — 自动合并为完整视频，画质无损、速度快
- **断点续传** — 失败 / 取消后保留进度，重试自动续传
- **下载历史** — 记录下载标题、时间、大小与文件名，防重复下载；支持单条删除与文件定位
- **剪贴板导入** — 可选开启：复制央视链接时自动弹窗提示导入（**默认关闭**，关闭时不读取剪贴板）
- **版本更新提醒** — 启动时后台检查新版本，有更新时在关于弹窗提示跳转（仅提示，不自动下载）
- **新内容提醒** — 收藏栏目有未下载新视频时，栏目名旁显示红点
- **现代化界面** — 垂直侧边栏（`Ctrl+\` 折叠、下载进度条与实时速度常驻）、深色模式、主题色自定义、封面大图预览、预览面板可折叠
...
| **单视频直链下载** | |
| <img src="docs/assets/single-video.png" width="400" alt="单视频直链下载"> | |
