...
- `src/main/download/coordinator.ts` — 下载任务调度，支持并行下载、队列排序、取消与断点续传，并根据媒体信息选择明文下载或 H5E 解密
- `src/main/download/decryptor.ts` — 并发处理明文或加密分片，共用重试、取消和残留文件清理策略；加密分片通过解密子进程处理
