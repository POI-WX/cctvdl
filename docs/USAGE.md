...
也支持 `v.cctv.com` 的节目目录链接。开启设置中的「加载节目看点和片段」后，正片、看点和片段会按发布时间统一排列，并用不同颜色的标签区分。该历史目录目前没有对应的看点和片段接口。
...
节目集不需要选择月份。打开后会按央视返回的内容总数逐页加载，直到得到完整选集，并即时显示已经加载的数量；选集较多时不必等待全部加载完成即可开始浏览。

可在列表上方切换「从早到晚」和「从晚到早」。节目集不显示「下载本月」，可下载单集或勾选后批量下载。
...
点击一条视频，右侧会显示封面、标题、简介、发布时间、频道和时长（以央视接口实际提供为准）。可直接复制标题或简介，也可把封面保存到本地；点击封面会打开大图预览。简介较长时可在简介区域内滚动，右下角的「下载此集 / 下载此视频」按钮会保持可见。
...
「保存设置」仅在设置发生修改后启用；保存完成后会恢复为不可用状态，并在按钮旁显示保存时间。
