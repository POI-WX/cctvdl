...
| 链接类型 | 导入后的使用方式 |
|---|---|
| **栏目** | 例如新闻联播。按月份浏览节目；可下载本月全部内容，也可选中多期后下载。央视中途更换过栏目 ID 的长期节目也会合并到同一个栏目。 |
| **节目集** | 电视剧、纪录片、4K 节目等。粘贴节目首页或任意一集都可以，直接显示整部节目的选集。 |
| **单个视频** | 电影、新闻文章视频、导视和精彩片段等。会进入左侧「📌 单个视频」集合。 |
| **央视新闻移动端视频** | 从央视新闻 App 或移动端分享出的可播放视频页；其中有多条视频时，会一并加入「单个视频」。 |
...
点击左侧栏目名称，选择月份后即可浏览该月节目。列表按从早到晚排列；可用月份旁的前后按钮快速切换，也可在搜索框按标题、简介或日期筛选。
...
点击一条视频，右侧会显示封面、标题、简介和发布时间。可直接复制标题或简介，也可把封面保存到本地；点击封面会打开大图预览。
...
常规央视节目会按所选档位下载；如果该档不存在，会选择接近的可用画质。央视新闻视频按页面实际提供的画质下载。

| 清晰度 | 大致分辨率 |
|---|---|
| 流畅 / 标清 / 高清 | 约 270p / 360p / 720p |
| 超清 | 约 720p，通常码率更高 |
| 蓝光 | 最高约 1080p |
| 自动 | 选择该节目可用的最高档位 |
...
| 剪贴板自动导入 | 默认关闭。开启后复制支持的央视链接会弹窗询问是否导入。 |
